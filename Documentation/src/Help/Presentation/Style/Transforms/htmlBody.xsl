<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
  ...
  <xsl:template name="memberFrameworksMenu">
    <div id="memberFrameworksMenu">
      <xsl:choose>
        <xsl:when test="/document/reference/elements/element[1]/versions/versions">
          <xsl:for-each select="/document/reference/elements/element[1]/versions/versions">
            <xsl:call-template name="FrameworksMenuCheckbox"/>
          </xsl:for-each>
        </xsl:when>
        <xsl:when test="/document/reference/elements/element[1]/element/versions/versions">
          <xsl:for-each select="/document/reference/elements/element[1]/element[1]/versions/versions">
            <xsl:call-template name="FrameworksMenuCheckbox"/>
          </xsl:for-each>
        </xsl:when>
      </xsl:choose>
